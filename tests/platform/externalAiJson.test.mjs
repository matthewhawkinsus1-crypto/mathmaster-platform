...
test('outside AI parser repairs unescaped LaTeX inequality commands inside JSON strings', () => {
  const raw = String.raw`{"schemaVersion":5,"assignment":{"title":"Honors","courseId":"algebra2"},"sections":[{"id":"practice","role":"practice","questions":[{"type":"multiAnswer","prompt":"For p(x)=|x|, use -4\\le x\\le 7."}]}]}`;
  const parsed = parseExternalAiJson(raw);
  assert.equal(parsed.sections[0].questions[0].prompt, String.raw`For p(x)=|x|, use -4\le x\le 7.`);
});

test('outside AI parser protects LaTeX commands that JSON would otherwise treat as control escapes', () => {
  const raw = String.raw`{"prompt":"Rewrite \\frac{1}{2}x using \\text{equivalent form}, then \\right-align it."}`;
  const parsed = parseExternalAiJson(raw);
  assert.equal(
    parsed.prompt,
    String.raw`Rewrite \frac{1}{2}x using \text{equivalent form}, then \right-align it.`,
  );
...
test('outside AI parser preserves already valid JSON escapes and unicode escapes', () => {
  ...
  assert.equal(parsed.latex, String.raw`\sqrt{x}`);
});

test('outside AI parser accepts fenced JSON and surrounding AI prose', () => {
  const fenced = String.raw`\`\`\`json
...
test('outside AI parser does not hide malformed JSON structure', () => {
  assert.throws(
    () => parseExternalAiJson(String.raw`{"schemaVersion":5,"sections":[}`),
    /not valid JSON/,
  );
});
