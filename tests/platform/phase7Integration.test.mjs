...
test('demo seed is populated, imperfect, resettable by design, and isolated from production collection names', () => {
  ...
  assert.equal(seed.students.every((student) => student.mathPath.current && Array.isArray(student.mathPath.questions) && student.mathPath.questions.length > 0), true);
  assert.equal(seed.students.filter((student) => !student.isFreshAccount).every((student) => student.mathPath.history.length > 0), true);
  assert.equal(seed.students.some((student) => student.isFreshAccount && student.mathPath.history.length === 0), true);
