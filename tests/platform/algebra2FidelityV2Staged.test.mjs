...
test('A2.5E determines logarithmic-solution reasonableness from original-equation evidence', async () => {
  const entry = payload('A2.5E');
  assert.ok(entry);
  assert.equal(entry.verdict, 'REBUILD');
  assert.match(entry.certificationStatus, /reasonableness-from-original-log-equation-domain-value-context-evidence/);
  ...
  for (const doc of entry.documents) {
    representations.add(doc.representation);
    const ids = new Set((doc.responseFields || []).map((field) => field.id));
    assert.ok(ids.has('verdict'), `${doc.id} must finish with a reasonableness verdict`);
    assert.ok(
      ids.has('argument') || (ids.has('arg1') && ids.has('arg2')),
      `${doc.id} must make the student evaluate the original logarithm argument(s)`,
    );

    const verdict = (doc.responseFields || []).find((field) => field.id === 'verdict')?.expected;
    if (verdict === 'keep') keepFamilies += 1;
    if (verdict === 'reject') rejectFamilies += 1;
    if (doc.id.includes('domain-invalid')) domainInvalidFamilies += 1;
    if (doc.id.includes('positive-domain-but-wrong')) positiveButWrongFamilies += 1;
    if (doc.id.includes('two-log')) multiLogFamilies += 1;
    if (doc.representation === 'context') contextFamilies += 1;
    ...
    for (const generated of samplePathInstances(doc, 40)) {
      ...
      if (doc.id.includes('common-log-valid')) {
        ...
        const logValue = Number(fields['log-value']?.expected);
        assert.ok(argument > 0);
        assert.equal(argument, 10 ** logValue);
        assert.equal(fields.domain?.expected, 'valid');
        assert.equal(fields.verdict?.expected, 'keep');
      }
      ...
      if (doc.id.includes('positive-domain-but-wrong')) {
        ...
        assert.ok(argument > 0);
        assert.equal(fields.domain?.expected, 'valid');
        assert.notEqual(lhs, rhs, `${doc.id} promised a positive-domain candidate that fails the original equation`);
        assert.equal(fields.verdict?.expected, 'reject');
      }
      ...
      if (doc.id.includes('two-log')) {
        ...
        const lhs = Number(fields.lhs?.expected);
        assert.ok(arg1 > 0 && arg2 > 0);
        assert.equal(arg1, 2 ** log1);
        assert.equal(arg2, 2 ** log2);
        assert.equal(lhs, log1 + log2);
        assert.equal(fields.domain?.expected, 'yes');
        assert.equal(fields.verdict?.expected, 'keep');
      }
      ...
      if (!spoiledChecked) {
        const verdictField = grading.fields.find((field) => field.id === 'verdict');
        assert.ok(verdictField);
        const wrongVerdict = String(verdictField.expected) === 'keep' ? 'reject' : 'keep';
        const wrong = await gradeResponse(grading, {
          responses: { ...responses, verdict: wrongVerdict },
        });
        assert.equal(wrong.isCorrect, false, `${doc.id} accepted the opposite reasonableness verdict`);
        wrongVerdictRejected += 1;
        spoiledChecked = true;
      }
  ...
  assert.ok(generatedCount >= 200);
  assert.ok(keepFamilies >= 2, 'A2.5E must include reasonable candidates, not only rejection practice');
  assert.ok(rejectFamilies >= 3, 'A2.5E must repeatedly reject unreasonable proposed solutions for distinct reasons');
  assert.ok(domainInvalidFamilies >= 1, 'A2.5E must include a candidate outside the logarithm domain');
  assert.ok(positiveButWrongFamilies >= 1, 'A2.5E must prove positive log arguments are not sufficient for equation validity');
  assert.ok(multiLogFamilies >= 1, 'A2.5E must check every argument in a multiple-log original equation');
  assert.ok(contextFamilies >= 1, 'A2.5E must separate algebraic validity from contextual reasonableness');
  assert.ok(errorFamilies >= 1, 'A2.5E must repair an incomplete reasonableness argument');
  assert.equal(wrongVerdictRejected, entry.documents.length);
  assert.ok(representations.has('symbolic'));
  assert.ok(representations.has('table'));
  assert.ok(representations.has('multipleRepresentation'));
