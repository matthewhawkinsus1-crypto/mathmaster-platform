...
test('calculator 2.0 stays inside Work View and publishes the discovery workflow', async () => {
  ...
  assert.match(component, /conversionAvailable \? <button[\s\S]*Convert to table/);
  assert.match(component, /type: 'table', rows: \[pair\.map\(String\),/);
  assert.match(component, /isRegressionExpression\(row\.value\) && tableRow \?/);
