...
const EXPECTED_STAGED = [
  ...
  'A.11B', 'A.12A', 'A.12C', 'A.12D',
  'A.2C', 'A.2D', 'A.2E', 'A.2F', 'A.2H', 'A.2I',
];

test('current staged Fidelity V2 candidate replaces nineteen complete standards only', () => {
  assert.deepEqual([...overrideCodes].sort(), [...EXPECTED_STAGED].sort());
  assert.equal(replacements.length, 95);
...
test('unstaged Algebra I families are carried forward without mutation', () => {
  ...
  assert.equal(baseUnstaged.length, 150);
  assert.equal(candidateUnstaged.length, 150);
