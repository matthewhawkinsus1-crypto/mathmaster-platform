...
const EXPECTED_STAGED = [
  ...
  'A.12D', 'A.2C', 'A.2H', 'A.2I',
];

test('current staged Fidelity V2 candidate replaces eleven complete standards only', () => {
  assert.deepEqual([...overrideCodes].sort(), [...EXPECTED_STAGED].sort());
  assert.equal(replacements.length, 55);
...
test('unstaged Algebra I families are carried forward without mutation', () => {
  ...
  assert.equal(baseUnstaged.length, 190);
  assert.equal(candidateUnstaged.length, 190);
