...
const bankFile = path.join(repoRoot, 'drafts', 'ccmr-v2.1', 'tsia2', 'algebraicReasoning', 'TSIA2_NATIVE_nonlinearEquationsFunctions.v2.1.json');
...
const assertSemantics = (template, scope) => {
  switch (template.taskType) {
    case 'quadraticAxisOfSymmetry':
      assert.equal(scope.answer, scope.h);
      break;
    case 'quadraticVertexCoordinate':
      assert.equal(scope.answerX, scope.h);
      assert.equal(scope.answerY, scope.k);
      break;
    case 'quadraticZeroFromFactors':
      assert.equal(scope.answer, Math.max(scope.r, scope.s));
      break;
    case 'exponentialFunctionValue':
      assert.ok(close(scope.answer, scope.initial * (scope.factor ** scope.x)));
      break;
    case 'exponentialInputFromOutput':
      assert.ok(close(scope.target, scope.initial * (scope.factor ** scope.answer)));
      break;
    case 'quadraticSymmetricInput':
      assert.equal(scope.mirrorX, 2 * scope.h - scope.x1);
      assert.equal(scope.y1, scope.a * ((scope.x1 - scope.h) ** 2) + scope.k);
      assert.equal(scope.mirrorY, scope.y1);
      break;
    case 'quadraticYInterceptFromRoots':
      assert.equal(scope.answer, scope.a * scope.r * scope.s);
      break;
    case 'exponentialIntervalFactor':
      assert.ok(close(scope.answer, scope.factor ** scope.delta));
      assert.ok(close(scope.laterValue, scope.startValue * scope.answer));
      break;
    default:
...
test('TSIA2 nonlinear-equations/functions bank has five direct and three independently authored challenge families', () => {
  assert.equal(bank.framework, 'tsia2');
  assert.equal(bank.domainId, 'algebraicReasoning');
  assert.equal(bank.nativeSkillId, 'nonlinearEquationsFunctions');
...
test('TSIA2 nonlinear-equations/functions bank passes 2,000 generated semantic checks', () => {
  let generated = 0;
  for (const template of bank.documents) {
    for (let sample = 0; sample < 250; sample += 1) {
      const instance = generatePathInstance(template, `tsia2-nef-semantic-${sample}`);
