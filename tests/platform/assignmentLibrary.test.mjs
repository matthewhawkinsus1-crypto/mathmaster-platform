...
const CLASS_PERIODS = ['1st', '2nd', '3rd'];
const courseProfiles = {
  '1st': { course: 'algebra1', courseLevel: 'standard' },
  '2nd': { course: 'algebra1', courseLevel: 'honors' },
  '3rd': { course: 'algebra1', courseLevel: 'standard' },
};
...
test('Acceptance 1 — no classes and blank dates is a library save, not an error', () => {
  const draft = { title: 'Attributes and Relations of Functions', assignedClassPeriods: [] };
...
test('an assignment with no class periods reads as a library item', () => {
  assert.equal(isLibraryAssignment({ assignedClassPeriods: [] }), true);
  assert.equal(isLibraryAssignment({}), true);
  assert.equal(isLibraryAssignment({ assignedClassPeriods: ['1st'] }), false);
});

// --- Acceptance 3 to 6: what assigning requires -----------------------------

test('Acceptance 3 — selecting a class makes the due date required', () => {
  const draft = { title: 'T', assignedClassPeriods: ['1st'] };
...
test('Acceptance 4 — a class plus a valid due date is enough', () => {
  const draft = { title: 'T', assignedClassPeriods: ['1st'], dueAt: '2026-09-14T15:30' };
  assert.deepEqual(collectReviewBlockers({ draft, classPeriods: CLASS_PERIODS }), []);
});
...
test('Acceptance 6 — a late date on or before the due date is rejected', () => {
  ...
  const draft = { title: 'T', assignedClassPeriods: ['1st'], dueAt: '2026-09-14T15:30', lateDueAt: '2026-09-14T15:30' };
...
test('Acceptance 7 — a library item cannot be posted to Google Classroom', () => {
  assert.throws(() => assertPublishable({ assignedClassPeriods: [], dueAt: null }), /not assigned to any class/);
  assert.equal(canPublishToClassroom({ assignedClassPeriods: [], dueAt: null }), false);
  // Assigned but somehow dateless is also refused, with its own reason.
  assert.throws(() => assertPublishable({ assignedClassPeriods: ['1st'], dueAt: null }), /no due date/);
  assert.equal(canPublishToClassroom({ assignedClassPeriods: ['1st'], dueAt: '2026-09-14T15:30:00Z' }), true);
});

// --- Acceptance 8: the split logic is shared, not duplicated ----------------

test('Acceptance 8 — one destination for a single-rigor selection', () => {
  const groups = buildDestinationGroups({ assignedClassPeriods: ['1st', '3rd'], courseProfiles });
...
test('Acceptance 8 — a mixed Standard/Honors selection splits into two', () => {
  const groups = buildDestinationGroups({ assignedClassPeriods: ['1st', '2nd'], courseProfiles });
...
test('a library save produces no destinations, which is the correct answer', () => {
  ...
  assert.deepEqual(buildDestinationGroups({ assignedClassPeriods: [], courseProfiles }), []);
});
...
test('an unknown class period falls back to standard rather than throwing', () => {
  const groups = buildDestinationGroups({ assignedClassPeriods: ['9th'], courseProfiles });
  assert.equal(groups.length, 1);
  assert.equal(groups[0].course, 'algebra1');
  assert.equal(groups[0].courseLevel, 'standard');
});
...
test('the title is still required for both actions', () => {
  ['library', 'assign'].forEach((mode) => {
    const draft = {
      title: '   ',
      assignedClassPeriods: mode === 'assign' ? ['1st'] : [],
...
test('the Honors rigor blocker still fires', () => {
  const blockers = collectReviewBlockers({
    draft: { title: 'T', assignedClassPeriods: ['2nd'], dueAt: '2026-09-14T15:30' },
...
test('the DOL window blocker still fires', () => {
  const blockers = collectReviewBlockers({
    draft: { title: 'T', assignedClassPeriods: ['1st'], dueAt: '2026-09-14T15:30', dolEnabled: true, dolMinutesBeforeEnd: 90 },
    classPeriods: CLASS_PERIODS,
  });
