...
test('StepByStep records the pre-commit equation for balanced moves and student rewrites', async () => {
  const source = await read('src/StepByStepAlgebraCore.jsx');
...
test('MultiRelation commits one history snapshot only after integrity validation and reset clears it', async () => {
  ...
  const resetBlock = blockBetween(source, 'const reset = () =>', '  const renderCandidateVerification');
