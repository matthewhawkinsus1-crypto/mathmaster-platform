...
const bankFile = path.join(repoRoot, 'drafts', 'ccmr-v2.1', 'tsia2', 'geometricSpatial', 'TSIA2_NATIVE_commonMeasurementUnits.v2.1.json');
const bank = JSON.parse(readFileSync(bankFile, 'utf8'));

const expectedCorrectLabel = {
  flagpoleLengthUnit: 'meters',
  townDistanceUnit: 'miles',
  smallLiquidVolumeUnit: 'milliliters',
  textbookMassUnit: 'kilograms',
  notebookThicknessUnit: 'millimeters',
};
...
const assertSemantics = (template, scope, question) => {
  ...
  if (expectedCorrectLabel[template.taskType]) {
    assert.equal(correct, expectedCorrectLabel[template.taskType]);
    return;
  }
  switch (template.taskType) {
    case 'reasonableHumanHeight':
      assert.ok(scope.height >= 5 && scope.height <= 7);
      assert.equal(correct, `${scope.height} feet`);
      break;
    case 'reasonableDrinkCapacity':
      assert.ok(scope.capacity >= 250 && scope.capacity <= 900);
      assert.equal(correct, `${scope.capacity} milliliters`);
      break;
    case 'floorAreaUnit':
      assert.ok(scope.area >= 100 && scope.area <= 500);
      assert.equal(correct, `${scope.area} square feet`);
      break;
    default:
...
test('TSIA2 common-measurement-units bank has five direct and three independently authored challenge families', () => {
  assert.equal(bank.framework, 'tsia2');
  assert.equal(bank.domainId, 'geometricSpatial');
  assert.equal(bank.nativeSkillId, 'commonMeasurementUnits');
...
test('TSIA2 common-measurement-units bank passes 2,000 generated semantic checks', () => {
  let generated = 0;
  for (const template of bank.documents) {
    for (let sample = 0; sample < 250; sample += 1) {
      const instance = generatePathInstance(template, `tsia2-common-measurement-units-${sample}`);
