// Build the direct ACT Math Path bank from the verified generative
// course bank. This is not a relabel operation: every output is converted to
// one of the two ACT response formats (4-option MCQ or numeric SPR),
...
const OUTPUT = path.join(ROOT, 'drafts/act.json');
const FRAMEWORK = 'act';
const TARGET_SPR = 0; // ACT Math uses multiple-choice items; no student-produced response items.
...
const actCodes = new Set(Object.keys(TEKS_EXAM_CROSSWALK).filter((code) => getExamDomainIds(code, FRAMEWORK).length));
...
for (const name of readdirSync(SEED_DIR).filter((entry) => entry.endsWith('_pathQuestionBank_seed.json')).sort()) {
  ...
  for (const q of documentsIn(parsed)) {
    ...
    if (actCodes.has(codeOf(q))) courseQuestions.push(q);
  }
}

if (courseQuestions.length !== 1125) {
  throw new Error(`Expected 1125 ACT-mapped course families, found ${courseQuestions.length}.`);
}
...
const ensureDerived = (q, baseName) => {
  ...
  const stem = `act_${safe(baseName).slice(0, 24)}`;
  let index = 1;
  let p1 = `${stem}_p1`;
...
const choiceField = () => ({ id: 'answer', label: 'Choose the correct answer', inputProfile: 'choice', expected: 'act-correct' });
...
const makeChoicesFromExpected = (q, profile, expected) => {
  ...
  if (profile === 'inequality') {
    ...
    if (present) {
      ...
      return [
        { id: 'act-correct', label: correct },
        { id: 'act-d1', label: alternatives[0] },
        { id: 'act-d2', label: alternatives[1] },
        { id: 'act-d3', label: alternatives[2] },
      ];
    }
  }
  ...
  if (!hit) {
    ...
    return [
      { id: 'act-correct', label: correct },
      { id: 'act-d1', label: fallback[0] },
      { id: 'act-d2', label: fallback[1] },
      { id: 'act-d3', label: fallback[2] },
    ];
  }
  ...
  return [
    { id: 'act-correct', label: correct },
    { id: 'act-d1', label: variants[0] },
    { id: 'act-d2', label: variants[1] },
    { id: 'act-d3', label: variants[2] },
  ];
};
...
const makeExistingChoiceFour = (q, source) => {
  ...
  const options = [{ id: 'act-correct', label: correct.label }];
  for (const choice of original) {
    ...
    used.add(label);
    options.push({ id: `act-d${options.length}`, label });
  ...
  // parameter (the first ACT draft correctly failed on exactly that defect).
  const bound = new Set([
...
const baseItem = (source, familyIndex) => {
  ...
  const out = {
    id: `mm_act_${safe(code)}_${familyIndex + 1}_${safe(sourceSlug)}`,
    active: true,
    ...
    familyId: `mathmaster:act:${code}:${sourceSlug}`,
    familyVersion: 1,
...
// First pass: normalize every course family to one ACT-answerable field. We do
...
for (const source of courseQuestions) {
  ...
  q.solutionReview = {
    ...(q.solutionReview || {}),
    headline: `Efficient ACT route: ${String(q.solutionReview?.headline || 'use the defining relationship').replace(/[.]$/, '')}.`,
  };
...
for (const entry of normalized) {
  ...
  if (sprIds.has(q.id)) {
    ...
    if (sourceWasChoice) {
      ...
      q.choices = fixedChoices.map((label, index) => ({ id: index === 0 ? 'act-correct' : `act-d${index}`, label }));
    } else {
...
if (byCode.size !== 225 || badFamilies.length) {
  throw new Error(`ACT family count mismatch: ${byCode.size} standards; bad=${JSON.stringify(badFamilies.map(([c, qs]) => [c, qs.length]))}`);
}
...
if (mcq.length !== 1125 || spr.length !== 0) throw new Error(`ACT format mismatch MCQ=${mcq.length}, SPR=${spr.length}`);
if (mcq.some((q) => q.choices?.length !== 4)) throw new Error('Every ACT MCQ must have exactly four options.');

const ACT_FULL_CHOICE_OVERRIDES = {
  'mm_act_A2_2A_3_cube_root_graph': ['all real numbers', '$x \\ge 0$', '$x > 0$', '$x \\ne 0$'],
  'mm_act_8_3B_5_compare_attributes': [
  ...
  'mm_act_6_8A_2_triangle_inequality': [
  ...
  'mm_act_A_12E_1_solve_linear_literal': [
    '$P/{{a}}-w$',
    '$P/{{a}}+w$',
    '$P-{{a}}w$',
    '$(P-w)/{{a}}$',
  ],
  'mm_act_A_12E_4_solve_area_height': [
    '$A/({{k}}b)$',
    '$A/{{k}}-b$',
    '${{k}}A/b$',
    '$A/({{k}}+b)$',
...
const ACT_CHOICE_COMPLETIONS = {
  'mm_act_A2_6K_4_notation_error': ['Only values $x<{{h}}$ are allowed.'],
  'mm_act_6_4B_3_compare_unit_rates': ['Both stores have the same cost per item.'],
  'mm_act_6_4C_4_same_attribute_check': ['${{a}}$ red marbles to ${{b}}$ kilograms'],
  'mm_act_6_4D_3_ratio_or_rate': ['A percent', 'A scale factor'],
  'mm_act_6_4G_5_equivalent_form_choice': ['${{dec}}\\%$'],
  'mm_act_6_6B_5_choose_equation': ['$y=x/{{k}}$'],
  'mm_act_6_6C_5_representation_match': ['$y=x/{{k}}$', '$y={{k}}+x$'],
  'mm_act_6_8B_5_formula_from_decomposition': ['${{b2}}{{h}}/2$'],
  'mm_act_6_8C_4_trapezoid_equation': ['$({{b1}}+{{b2}})x/2=2{{A}}$'],
  'mm_act_6_10B_1_equation_true': ['The equation has no solution.'],
  'mm_act_6_10B_2_equation_false': ['Every value of $x$ makes the equation true.'],
  'mm_act_6_10B_3_inequality_true': ['The inequality has no solutions.', 'Every real number satisfies the inequality.'],
  'mm_act_6_10B_4_inequality_boundary': ['The inequality has no solutions.', 'Only values less than {{a}} satisfy it.'],
  'mm_act_6_12A_5_choose_display': ['Scatterplot'],
  'mm_act_6_12C_5_summary_comparison': ['The data sets have the same range.', 'Neither data set has a range.'],
  'mm_act_6_12D_4_percent_bar_largest': ['A and C are tied.'],
  'mm_act_6_13A_5_boxplot_center_compare': ['The distributions have the same median.', 'Neither distribution has a median.'],
  'mm_act_7_4A_4_match_equation': ['$y={{r}}$'],
  'mm_act_7_4B_5_compare_unit_rates': ['The plans have the same cost per unit.', 'Both plans cost $0$ per unit.'],
  'mm_act_7_4C_5_compare_k': ['The constants of proportionality are equal.', 'Both constants of proportionality are 0.'],
  'mm_act_7_4E_5_choose_conversion': ['${{inch}}\\times25$'],
  'mm_act_7_5B_5_compare_circles': ['Both ratios are approximately 1.'],
  'mm_act_7_5C_5_compare_scales': ['The drawing lengths are equal.', 'Both drawing lengths are 0.'],
  'mm_act_7_6A_4_missing_outcome': ['A list with exactly {{s}} total outcomes'],
  'mm_act_7_6C_5_reasonableness': ['About 0 occurrences'],
  'mm_act_7_6D_5_compare_probabilities': ['The events are equally likely.', 'Neither event can occur.'],
  'mm_act_7_6E_5_complement_check': ['No, because complementary events must have equal probabilities.', 'Yes, but only when $P(E)=0$.'],
  'mm_act_7_7_4_compare_representations': ['The relationships have the same rate of change.', 'Their rates of change cannot be determined from the information given.'],
  'mm_act_7_8A_1_volume_ratio': ['The pyramid volume is three times the prism volume.'],
  'mm_act_7_9A_5_compare_solids': ['The prism has one-third the volume of the pyramid.'],
  'mm_act_7_9B_5_area_vs_circumference_formula': ['$\\pi{{r}}$', '$2\\pi({{r}})^2$'],
  'mm_act_7_9C_5_decomposition_choice': ['${{w}}{{h}}+({{b}}+{{t}})/2$'],
  'mm_act_7_9D_5_net_method': ['$2lw+2h$'],
  'mm_act_7_10A_4_match_model': ['${{m}}x={{total}}+{{b}}$'],
  'mm_act_7_11B_3_boundary_inclusive': ['No value of $x$ satisfies the inequality.'],
  'mm_act_7_11C_5_angle_equation_choice': ['$x+{{a}}=360$'],
  'mm_act_7_12B_3_sample_method': ['Select the first residents alphabetically from the city list.'],
  'mm_act_7_13A_5_compare_tax_rates': ['The two purchases have equal tax.', 'No tax is collected on either purchase.'],
  'mm_act_7_13E_4_compare_interest': ['Neither account earns interest.'],
  'mm_act_8_2B_5_pi_comparison': ['$\\pi={{n}}/100$', '$\\pi=0$'],
  'mm_act_8_2C_3_valid_form': ['$10^{{e}}+{{c}}$'],
  'mm_act_8_2C_5_compare_magnitudes': ['The quantities are equal.', 'Quantity A is 10 times smaller than Quantity B.'],
  'mm_act_8_3A_5_proportion_check': ['${{ak}}/{{a}}={{c}}/{{ck}}$', '${{ak}}+{{a}}={{ck}}+{{c}}$'],
  'mm_act_8_3C_5_error_translation': ['It is a rotation because the coordinates are multiplied by {{k}}.', 'It is a reflection because every coordinate changes sign.'],
  'mm_act_8_4A_5_similar_triangles': ['The second triangle has twice the slope of the first.', 'Slope cannot be compared using rise and run.'],
  'mm_act_8_4B_5_proportional_check': ['$y=x+{{k}}$', '$y={{k}}$'],
  'mm_act_8_4C_5_error_rate': ['Divide the run {{dx}} by the rise {{rise}}.', 'Use only the $y$-intercept to determine slope.'],
  'mm_act_8_5A_5_compare_representations': ['$y=x+{{k}}$', '$y={{k}}$'],
  'mm_act_8_5B_5_contrast_proportional': ['$y=({{m}}+1)x$', '$y=x/{{m}}$'],
  'mm_act_8_5D_5_extrapolation_error': ['Predictions are always more accurate outside the observed range.', 'The input must be changed to {{hi}} before the model can be used.'],
  'mm_act_8_5E_5_error_intercept': ['Direct variation must have slope 0.', 'A direct-variation relationship cannot be graphed.'],
  'mm_act_8_5I_5_error_model': ['$y={{b}}-{{r}}$'],
  'mm_act_8_6A_5_formula_meaning': ['Volume is circumference times height.', 'Volume is base area divided by height.'],
  'mm_act_8_6C_5_error_lengths': ['${{a}}^2+{{b}}^2=({{a}}+{{b}})^2$'],
  'mm_act_8_7A_5_compare_solids': ['The solids have equal volume.', 'The cone volume is three times the cylinder volume.'],
  'mm_act_8_7B_5_lateral_vs_total': ['Total surface area equals lateral area minus the two bases.', 'Lateral area includes the bases but total surface area does not.'],
  'mm_act_8_7C_5_error_sum': ['Check whether ${{a}}^2+{{b}}^2=({{a}}+{{b}})^2$.'],
  'mm_act_8_7D_5_error_manhattan': ['Use the larger of {{a}} and {{b}} as the distance.', 'Use $|{{a}}-{{b}}|$ as the distance.'],
  'mm_act_8_8A_2_verbal_match': ['${{m}}x-{{a}}={{n}}x+{{b}}$', '${{m}}x+{{a}}={{n}}x-{{b}}$'],
  'mm_act_8_8A_5_error_sign': ['${{start}}-{{d}}t={{other}}-{{f}}t$', '${{start}}+{{d}}t={{other}}-{{f}}t$'],
  'mm_act_8_8C_5_special_case': ['Exactly two solutions'],
  'mm_act_8_9_5_parallel_error': ['Infinitely many intersections.', 'Exactly two intersections.'],
  'mm_act_8_10C_2_reflection_name': ['A $180^\\circ$ rotation about the origin'],
  'mm_act_8_10D_5_error_area_factor': ['Area is multiplied by ${{k}}^3$.', 'Area is multiplied by $2{{k}}$.'],
  'mm_act_8_11B_5_compare_spread': ['The data sets have equal spread.', 'Neither data set has measurable spread.'],
  'mm_act_8_12B_5_compare_total_cost': ['The loans have equal total repayment.', 'Both total repayments are $0$.'],
  'mm_act_8_12C_5_early_saving': ['Fewer contributions but more time for growth.', 'The same number of contributions and the same growth time.'],
  'mm_act_8_12D_5_compare_methods': ['The methods always earn exactly the same amount.', 'Neither method earns interest.'],
};

// Human-authored ACT editorial overrides. These are intentionally small and
...
const addActConstraints = (id, constraints) => {
...
// while guaranteeing that all four ACT options remain distinct.
addActConstraints('mm_act_A2_5C_1_exp_to_log', ['x >= 2', 'b != x']);
addActConstraints('mm_act_A2_5C_2_log_to_exp', ['x >= 2', 'b != x']);
addActConstraints('mm_act_A2_5C_5_reverse_role', ['b != x', 'x >= 2']);
addActConstraints('mm_act_A2_3B_3_matrix_check', ['x != y', 'y != z', 'x != 0']);
addActConstraints('mm_act_A2_8B_3_exponential_regression_perfect', ['a != r']);
addActConstraints('mm_act_A2_2A_4_reciprocal_asymptotes', ['h != 0', 'k != 0', 'h != k']);
addActConstraints('mm_act_A2_4A_5_choose_equation_from_points', ['b != c']);
addActConstraints('mm_act_A2_7G_5_reverse_radical', ['k != p']);
addActConstraints('mm_act_A2_4B_1_vertex_focus', ['h != k']);
addActConstraints('mm_act_A2_6G_2_reciprocal_asymptotes', ['h != 0', 'k != 0', 'h != k']);
addActConstraints('mm_act_8_8A_2_verbal_match', ['a != b']);
addActConstraints('mm_act_A2_6C_4_absolute_shift_error', ['h != 0', 'h != k']);
addActConstraints('mm_act_A2_7B_5_degree_leading', ['a != 3']);
addActConstraints('mm_act_A2_7B_4_area_model', ['dl + dw != dl * dw']);
addActConstraints('mm_act_A2_6D_3_two_solutions_to_equation', ['center != 0', 'center != d']);
addActConstraints('mm_act_A2_6G_4_reciprocal_shift_error', ['h != k']);
addActConstraints('mm_act_A2_3A_3_from_table_equations', ['m != a || b != c']);
addActConstraints('mm_act_A2_5B_3_recursive_to_explicit', ['a0 != r']);
addActConstraints('mm_act_7_10A_4_match_model', ['m != b']);
addActConstraints('mm_act_7_10C_1_equation_scenario', ['m != b']);
addActConstraints('mm_act_8_3A_5_proportion_check', ['a != c']);
addActConstraints('mm_act_6_2E_4_meaning_of_fraction', ['a != b']);
addActConstraints('mm_act_6_3A_4_equivalent_operation', ['p != q']);
const perpendicularContext = byId.get('mm_act_A_2F_3_context_perpendicular');
if (perpendicularContext) perpendicularContext.generator.parameters.m.max = 12;
const literalProduct = byId.get('mm_act_A_12E_2_solve_product_literal');
if (literalProduct) literalProduct.generator.parameters.a.max = 14;
const compareDilationAttributes = byId.get('mm_act_8_3B_5_compare_attributes');
if (compareDilationAttributes) compareDilationAttributes.generator.parameters.k.max = 12;
const solveLinearLiteral = byId.get('mm_act_A_12E_1_solve_linear_literal');
if (solveLinearLiteral) solveLinearLiteral.generator.parameters.a.max = 14;
const solveAreaHeight = byId.get('mm_act_A_12E_4_solve_area_height');
if (solveAreaHeight) solveAreaHeight.generator.parameters.k.max = 14;
const additiveTable = byId.get('mm_act_6_4A_4_table_additive');
if (additiveTable) {
...
for (const [id, labels] of Object.entries(ACT_FULL_CHOICE_OVERRIDES)) {
  const question = byId.get(id);
  if (!question) continue;
  question.choices = labels.map((label, index) => ({ id: index === 0 ? 'act-correct' : `act-d${index}`, label }));
  question.responseFields = [choiceField()];
}
for (const [id, labels] of Object.entries(ACT_CHOICE_COMPLETIONS)) {
  const question = byId.get(id);
  if (!question) continue;
  ...
  for (const label of labels) {
    ...
    if (used.has(label)) continue;
    used.add(label);
    retained.push({ id: `act-d${retained.length}`, label });
  }
  if (retained.length !== 4) throw new Error(`${id}: ACT distractor completion produced ${retained.length} options.`);
  retained[0].id = 'act-correct';
  for (let index = 1; index < retained.length; index += 1) retained[index].id = `act-d${index}`;
...
for (const question of normalizedOutput) {
  ...
  if (/\u000[0-8bcef]/i.test(raw)) throw new Error(`${question.id}: unsupported control character remained after ACT math normalization.`);
}

writeFileSync(OUTPUT, `${JSON.stringify({ documents: normalizedOutput }, null, 2)}\n`);
console.log(`Wrote ${output.length} ACT generator templates to ${path.relative(ROOT, OUTPUT)}.`);
