...
function App() {
  ...
  useEffect(() => {
    if (studentSpotlightRequest?.status !== SPOTLIGHT_STATUS.ACCEPTED) return undefined;
    const expiresAt = studentSpotlightRequest.expiresAt?.toMillis?.() || 0;
    const timer = window.setTimeout(() => stopStudentSpotlight(), Math.max(0, expiresAt - Date.now()));
    return () => window.clearTimeout(timer);
