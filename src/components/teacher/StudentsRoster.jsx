...
export default function StudentsRoster({
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter((student) => `${student.id} ${student.classPeriod || ''}`.toLowerCase().includes(needle));
  }, [students, search]);
  ...
  return (
    <div style={{ textAlign: 'left' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 14, alignItems: 'flex-end', flexWrap: 'wrap', marginBottom: 16 }}><div><h2 style={{ margin: 0 }}>Students</h2><p style={{ margin: '5px 0 0', color: '#5f6368' }}>Compact roster first. Open a student only when you need details.</p></div><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search student or class" style={{ width: 'min(360px, 100%)', padding: '10px 12px', border: '1px solid #c7cdd6', borderRadius: 8 }} /></div>
