...
export const buildAssignmentCreatorRequest = (input = {}, { generatedAt = new Date() } = {}) => {
  ...
  const outputNotes = [
    ...
    ...(plan.outputs.lessonNotesPdf ? ['- When lesson notes are enabled, set targetPages to 2 and author substantive student-facing notes now. Include a learning goal and at least two content-bearing sections; never return an enabled notes PDF with an empty sections array.'] : []),
  ];
  ...
  return [
    ...
    '- If domain/range is requested both as an inequality/set and in words, have the student produce the mathematical representation first; the verbal response should translate that work rather than reveal its bounds.',
    ...
    '- For a full Practice section of 5 or more questions, author the normal TEKS-aligned independent practice the lesson needs. Do NOT imitate SAT, ACT, TSIA2, or ASVAB wording merely to satisfy CCMR. If this assignment is sent to an Honors destination, MathMaster will source matching Practice items from the audited CCMR Fidelity V2.1 bank while preserving the same TEKS and the requested question count.',
    '- For Honors-ready Practice, preserve the MathMaster CCMR policy: audited exam-style transfer should be roughly 15% over the recent sequence, using only legitimate TEKS-to-assessment overlap.',
