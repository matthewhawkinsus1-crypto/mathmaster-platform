...
import { buildTransferUnit, createExportSnapshot, teamsCsv, TRANSFER_STATE, transferFileName } from '../../platform/gradeTransfer/gradeTransferModel.js';
...
export default function GradeTransferCenter({ classes, assignments, students, teacherUid, teacherEmail, calculateCanonicalGrade }) {
  const [snapshots, setSnapshots] = useState([]);
  const [selected, setSelected] = useState(new Set());
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState('');
  useEffect(() => { listTeacherTransferSnapshots(teacherUid).then(setSnapshots).catch((error) => setMessage(error.message)); }, [teacherUid]);
  const units = useMemo(() => (classes || []).flatMap((classRecord) => (assignments || [])
    .filter((assignment) => assignment.classId === classRecord.classId || (!assignment.classId && assignment.classPeriod === classRecord.period) || assignment.classIds?.includes(classRecord.classId))
    .map((assignment) => {
      const eligible = (students || []).filter((student) => student.classId === classRecord.classId || (!student.classId && student.classPeriod === classRecord.period));
      const history = snapshots.filter((item) => item.classId === classRecord.classId && item.assignmentId === assignment.id).sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)));
      return buildTransferUnit({ classRecord, assignment, students: eligible, calculateCanonicalGrade, confirmedSnapshot: history.find((item) => item.uploadConfirmedAt), latestExport: history[0] });
    })), [classes, assignments, students, snapshots, calculateCanonicalGrade]);
  ...
  const prepare = async (chosen) => {
    ...
    try {
      ...
      for (const unit of ready) {
        const snapshot = createExportSnapshot({ unit, transferId: id('transfer'), teacherUid, teacherEmail, packageId });
        await persistTransferSnapshot(snapshot); created.push(snapshot);
      }
      download(buildGradebookZip(ready), `${packageId}.zip`, 'application/zip');
      setSnapshots((current) => [...current, ...created]); setSelected(new Set());
  ...
  return <section aria-labelledby="grade-transfer-heading">
    ...
    <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr><th></th><th>Assignment</th><th>Class / period</th><th>Final deadline</th><th>Finalized</th><th>Extensions</th><th>Status</th><th>Changed</th><th>Next action</th></tr></thead><tbody>
      {units.map((unit) => { const latest = snapshots.filter((item) => item.classId === unit.classId && item.assignmentId === unit.assignmentId).sort((a,b) => String(b.createdAt).localeCompare(String(a.createdAt)))[0]; return <tr key={unit.key} style={{ borderTop: '1px solid #dadce0' }}>
        ...
        <td style={{ padding: 8 }}><button type="button" disabled={busy || !unit.rows.length || !downloadable.has(unit.state)} onClick={() => prepare([unit])}>Individual export</button>{latest && !latest.uploadConfirmedAt && <button type="button" disabled={busy} onClick={async () => { setBusy(true); try { await confirmTransferUploaded({ transferId: latest.transferId || latest.id, actorUid: teacherUid, actorEmail: teacherEmail }); setSnapshots(await listTeacherTransferSnapshots(teacherUid)); } catch (error) { setMessage(error.message); } finally { setBusy(false); } }}>Mark Uploaded to TEAMS</button>}</td>
      </tr>; })}
    </tbody></table></div>
