...
export default function RegressionCalculator({ questionData = {}, onAction }) {
  ...
  const sourceGraphBounds = useMemo(() => {
    const authored = questionData.sourceGraphBounds;
    if (authored && ['xMin','xMax','yMin','yMax'].every((key) => Number.isFinite(Number(authored[key])))) {
      return Object.fromEntries(['xMin','xMax','yMin','yMax'].map((key) => [key, Number(authored[key])]));
    }
    return boundsFor(source);
  }, [questionData.sourceGraphBounds, source]);
  ...
  const task = sourceMode === 'scatterplot'
    ? 'Read the plotted points from the axes, convert them to ordered pairs in the x₁/y₁ table, then use regression technology to produce r.'
    : 'Use the source data in the calculator workflow.';
  const steps = sourceMode === 'scatterplot'
    ? ['Read each point from the scatterplot and enter its ordered pair in the x₁/y₁ table.', 'Select and run linear regression.', 'Use the produced r to interpret direction and strength.']
    : ['Enter every x₁ and y₁ value.', 'Select and run linear regression.', 'Use the produced r to interpret direction and strength.'];
  ...
  return <ToolShell title="Regression Calculator" subtitle="Enter paired data, run linear regression, then interpret the correlation it produces." badge="Assessment statistics">
    <TaskCard question={questionData} task={task} steps={steps} />
    <div className="regression-workspace">
      {sourceMode === 'scatterplot' ? (
        <Panel title="Source scatterplot">
          <div data-regression-source-graph className="regression-source-graph">
            <CoordinatePlane
              {...sourceGraphBounds}
              ...
              enlargeable={false}
              panZoom={false}
              ariaLabel="Source scatterplot. Read point locations from the axes; coordinates are intentionally not revealed."
            />
          </div>
          <p className="regression-source-note">Read each point from the axes and enter the ordered pair in the table. Point coordinates are not revealed by hover or touch.</p>
        </Panel>
      ) : (
