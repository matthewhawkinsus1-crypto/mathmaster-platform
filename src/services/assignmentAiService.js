...
const authorHonorsAssignment = httpsCallable(functions, 'authorHonorsAssignmentWithGemini', {
  timeout: 300000,
});
...
const isHonorsDepthRequest = (value) => String(value || '').trimStart().startsWith('# MathMaster Honors-depth repair');
...
export async function buildAssignmentWithAI(prompt, { ccmrEnrichment = false } = {}) {
  const text = String(prompt || '').trim();
  if (!text) throw new Error('Finish the assignment plan before building with AI.');

  // Honors-depth repair is deliberately isolated from normal Assignment V5 AI.
  // The marker is generated by MathMaster's audited Honors repair contract; the
  // server independently re-checks it before Gemini is allowed to run.
  const author = isHonorsDepthRequest(text) ? authorHonorsAssignment : authorAssignment;
  const response = await author({ prompt: text, ccmrEnrichment: ccmrEnrichment === true });
