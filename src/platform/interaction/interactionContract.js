...
const workflowRecipeName = (question = {}) => {
  if (typeof question?.recipe === 'string') return clean(question.recipe);
  return clean(question?.recipe?.name || question?.recipe?.recipe);
};

const addSafeWorkflowProvenance = (question = {}) => {
  if (!isObject(question) || isObject(question.workflowProvenance)) return question;
  const workflow = Array.isArray(question.workflow) ? question.workflow : [];
  if (!workflow.length) return question;

  const type = clean(question.type).toLowerCase();
  const recipeName = workflowRecipeName(question);

  // Multi-stage graphChoicePreview is the compiler's narrow explicit-workflow
  // exception. A generated graph-choice preview has one `choice` stage, so two
  // or more stages are unambiguous authored workflow and must never be removed
  // by compatibility repair.
  if (type === 'graphchoicepreview' && workflow.length > 1) {
    return {
      ...question,
      workflowProvenance: {
        source: 'authored',
  ...
  // Function-modeling composed workflows are generated by the V5 compiler from
  // mathematical studentActions and the named functionModeling recipe. This is
  // the exact provenance needed by the deterministic no-synthetic-graph repair.
  // Do not infer provenance for a one-stage graph choice or another ambiguous
  // historical shape; absence remains the safe fail-closed state.
  if (type === 'functiongraph' && recipeName.toLowerCase() === 'functionmodeling' && workflow.length > 1) {
    return {
      ...question,
      workflowProvenance: {
...
export const normalizeQuestionInteractionContracts = (question = {}) => {
  ...
  return addSafeWorkflowProvenance(out);
};
