...
const cleanText = (value, maximum = 4000) => String(value ?? '')
  .replace(/https?:\/\/\S+/gi, '[link removed]')
  .slice(0, maximum);
const frameBytes = (frame) => new TextEncoder().encode(JSON.stringify(frame)).length;
...
export const buildSpotlightFrame = ({
  ...
  const candidates = [
    frame,
    { ...frame, work: { message: 'Detailed interactive work was compacted for projection.' }, truncated: true },
    {
      ...frame,
      question: { ...safeQuestion, table: null, graph: null },
      work: { message: 'Detailed interactive work was compacted for projection.' },
      truncated: true,
    },
    {
      ...frame,
      question: {
        type: cleanText(safeQuestion?.type || 'question', 80),
        toolId: cleanText(safeQuestion?.toolId || '', 80) || null,
        prompt: cleanText(safeQuestion?.prompt || 'Current question', 1200),
        directions: cleanText(safeQuestion?.directions || '', 400),
        table: null,
        graph: null,
      },
      work: { message: 'Detailed work was too large to project safely.' },
      truncated: true,
    },
    {
      schemaVersion: 1,
      ...
      questionIndex: Math.max(0, Math.floor(Number(questionIndex) || 0)),
      studentLabel: cleanText(studentLabel || 'Student', 80),
      question: { type: 'question', toolId: null, prompt: cleanText(safeQuestion?.prompt || 'Current question', 400), directions: '', table: null, graph: null },
      work: { message: 'This work was too large to project safely.' },
      updatedAtMs: Number(nowValue) || Date.now(),
      truncated: true,
    },
  ];
  return candidates.find((candidate) => frameBytes(candidate) <= SPOTLIGHT_MAX_FRAME_BYTES)
    || candidates.at(-1);
};
...
export const scheduleSpotlightExpiry = ({
  request,
  onExpire,
  nowValue = Date.now(),
  setTimer = setTimeout,
  ...
  const timer = setTimer(() => onExpire?.(), Math.max(0, expiresAt - nowValue));
