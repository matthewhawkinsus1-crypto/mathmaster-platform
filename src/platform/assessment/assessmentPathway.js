...
export const getAssessmentPathwayState = ({
  ...
  if (!retestOpen) {
    return {
      ...
      detail: 'Your test result qualifies for a retest. The retest will appear here when it opens.',
      passingScore,
      review,
      test,
      retest,
  ...
  if (retest.total > 0 && !retest.terminal) {
    return {
      ...
      statusLabel: 'Retest',
      detail: 'Retest mode uses a shorter fresh assessment. Your original test remains locked while you retest.',
      passingScore,
      review,
      test,
      retest,
