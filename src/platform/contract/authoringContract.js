...
const fidelitySection = () => section('Source representation fidelity', [
  ...
  '',
  line('**Reference information is not a hint or an answer key.** The blue Your Task card stays visible while students work.'),
  bullet('Omit `referenceInfo` when the prompt already contains the givens the student needs. Do not create a second card just to repeat the task in shorter sentences.'),
  bullet('Use `referenceInfo` only for source facts or data the student must repeatedly consult while working, such as a supplied formula, conversion, data value, or definition that is explicitly GIVEN rather than derived.'),
  bullet('Never place a student conclusion in `referenceInfo`: independent/dependent roles, the equation/model, domain, range, continuity, axis labels, scale, transformed values, table outputs, intercepts, extrema, or any other answer the student is being asked to determine.'),
  bullet('If the source explicitly gives one of those facts and the question assesses something else, it may remain visible as a given. Otherwise the student must do that thinking in the workspace.'),
  '',
