...
export const buildAuthoringContract = ({ generatedAt = new Date(), courseId = null } = {}) => [
  ...
  '- If lessonNotesPdf.enabled is true, author the notes in the same JSON build: include a learningGoal and at least two content-bearing sections (for example key ideas/vocabulary plus a worked example or reference pattern). Never enable lessonNotesPdf with an empty sections array; blank placeholder notes must never be published to students or Google Classroom.',
