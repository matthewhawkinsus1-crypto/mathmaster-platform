...
export default function GraphStory({ question, onStateChange, onUndoStateChange, feedback, draftKey }) {
  ...
  const parts = [
    ...
    { id: 'graph-sketch', label: 'Graph sketch', isComplete: values.strokes.some((stroke) => stroke.length >= 3), isCorrect: values.strokes.some((stroke) => stroke.length >= 3), response: `${values.strokes.length} stroke(s)` },
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', textAlign: 'left' }}>
      <h2 style={{ textAlign: 'center', marginTop: 0 }}>Write a Scenario and Sketch Its Graph</h2>
