...
const canonicalSimpleInequality = (value) => {
  ...
  const direct = text.match(new RegExp(`^(${quantityPattern})(<=|>=|<|>)(-?\\d+(?:\\.\\d+)?)// Shared answer-equivalence rules used by the browser, Teacher Path Simulator,
// and Cloud Functions. A mathematically correct response must not be rejected
// merely because MathLive serializes it differently from the authored key.
//
// Keep deterministic equivalence here rather than in individual renderers. The
...
const invertInequalityOperator = (operator) => ({
  ...
  const reversed = text.match(new RegExp(`^(-?\\d+(?:\\.\\d+)?)(<=|>=|<|>)(${quantityPattern})// Shared answer-equivalence rules used by the browser, Teacher Path Simulator,
// and Cloud Functions. A mathematically correct response must not be rejected
// merely because MathLive serializes it differently from the authored key.
//
// Keep deterministic equivalence here rather than in individual renderers. The
...
const invertInequalityOperator = (operator) => ({
  ...
  const chained = text.match(new RegExp(`^(-?\\d+(?:\\.\\d+)?)(<=|>=|<|>)(${quantityPattern})(<=|>=|<|>)(-?\\d+(?:\\.\\d+)?)// Shared answer-equivalence rules used by the browser, Teacher Path Simulator,
// and Cloud Functions. A mathematically correct response must not be rejected
// merely because MathLive serializes it differently from the authored key.
//
// Keep deterministic equivalence here rather than in individual renderers. The
